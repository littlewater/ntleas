...
static DWORD CALLBACK InitUnicodeLayer(LPVOID param);
...
static BOOL DispatchToOtherProcessW(LPWSTR NtleasXPath, DWORD NtleasXSize, HINSTANCE hInstance, LPCWSTR commandstring) {
	...
	if (hfile != INVALID_HANDLE_VALUE) {
		...
		LPCWSTR realpath, _p = GetAppPathW(commandstring, &realpath);
		wchar_t code = *_p; *(LPWSTR)_p = L'\0'; // tmpsave
		if (tailpath <= _p) wsprintfW(ntleasx, L" \"%s\"", realpath); // only process 
		else wsprintfW(ntleasx, L" \"%s\" \"A%s\"", realpath, _p + 1); // not only process
	//	wsprintfW(NtleasXPath, L"%s %s", realpath, _p + 1);
		*(LPWSTR)_p = code; // restore
		// ---------------- 
		return (TRUE);
	}
	return (-1);
...
{
	NtleaProcess* psettings = (NtleaProcess*)lParam;
	...
	if (VerQueryValueA(pResCopy, "\\VarFileInfo\\Translation", (LPVOID*)&lpTranslate, &cbTranslate)) {
		if (cbTranslate >= sizeof(LANGANDCODEPAGE)) {
			psettings->dwLCID = lpTranslate[0].wLanguage;
			psettings->dwCodePage = lpTranslate[0].wCodePage;
		}
	}
	LocalFree(pResCopy);
